...
/**
 * Generates a single-sentence training recommendation based on Oura Readiness, HRV, and Sleep scores.
...
export async function generateAutoregulationRecommendation(
  ...
  const getHeuristicRecommendation = () => {
    if (readiness >= 85 && hrv >= 60) {
      ...
      return "Suggesting active recovery or rest. Central nervous system is fatigued; prioritize sauna, cold plunge, and sleep hygiene.";
    }
  };
...
Provide a single-sentence recommendation (under 25 words) advising whether they should go hard (strength/grappling), focus on technical/moderate work, or do active recovery/rest. Be direct, philosophical, and practical. Do not use quotes or introductory tags in the output.`,
    });
